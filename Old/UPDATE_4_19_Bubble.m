...
%5.18Pos改为180
%删除函数头以及最后的END，可单独使用
function a = UPDATE_4_19_Bubble(addr)
% addr = 'D:\Decode_File\Bubble\200610-1315-178.9953-Crossing Field.txt';
a = 0;
%

% addr = "D:\Decode_File\Idol\100442-1825-200-Gee.txt";
Score_Note_shot = 2600;
Score_Note_green = Score_Note_shot*0.3; % =780
Score_Note_blue = 2600;
tic
...
max_pos = max(data2{:, 3});
Total_Score_Green = zeros(max_pos, 2); 
Total_Score_Green(:, 2) = [0:max_pos-1]./8*60/bpm+et/1000;
Total_Score_Blue = Total_Score_Green;
Total_Score_Shot = Total_Score_Green;
Total_Score_Mix = Total_Score_Green;
for i = 1:size(data2, 1)    
    if strcmp(data2{i, 1}, '2')
        Total_Score_Blue(data2{i, 2} : 4 : data2{i, 3}, 1) = Total_Score_Blue(data2{i, 2} : 4 : data2{i, 3}, 1) + Score_Note_blue;
    elseif strcmp(data2{i, 1}, '0')
        Total_Score_Shot(data2{i, 2}, 1) = Total_Score_Shot(data2{i, 2}, 1) + Score_Note_shot;
    elseif strcmp(data2{i, 1}, '1')
        Total_Score_Green(data2{i, 2} : 4 : data2{i, 3}, 1) = Total_Score_Green(data2{i, 2} : 4 : data2{i, 3}, 1) + Score_Note_green;
    else
        error('Note Type Error')
    end
end
Total_Score_Mix(:, 1) = Total_Score_Green(:, 1) + Total_Score_Blue(:, 1) + Total_Score_Shot(:, 1);	%各类分数相加得出总分   

time = floor(max(Total_Score_Green(:, 2))) + 1;
time_hist_score_green = zeros(time, 1);
time_hist_score_shot = zeros(time, 1);
time_hist_score_blue = zeros(time, 1);
time_hist_score_mix = zeros(time, 1);
for i = 1:size(Total_Score_Green, 1)
    time_hist_score_green(floor(Total_Score_Green(i, 2))+1) = time_hist_score_green(floor(Total_Score_Green(i, 2))+1) + Total_Score_Green(i, 1);
    time_hist_score_shot(floor(Total_Score_Shot(i, 2))+1) = time_hist_score_shot(floor(Total_Score_Shot(i, 2))+1) + Total_Score_Shot(i, 1);
    time_hist_score_blue(floor(Total_Score_Blue(i, 2))+1) = time_hist_score_blue(floor(Total_Score_Blue(i, 2))+1) + Total_Score_Blue(i, 1);
    time_hist_score_mix(floor(Total_Score_Mix(i, 2))+1) = time_hist_score_mix(floor(Total_Score_Mix(i, 2))+1) + Total_Score_Mix(i, 1);
end
time_hist_score_dist = [time_hist_score_green';time_hist_score_shot';time_hist_score_blue']; %转换为图表数据格式

figure('Position', [0 0 1920 1080], 'Visible', 'off');
subplot(2,1,1)  %每秒note
fg = bar(0:time-1, time_hist_score_dist', 'stacked', 'EdgeColor', 'none');
set(fg(1),'facecolor',[0.5 1 0.5])
set(fg(2),'facecolor',[1 0.87 0.44])
set(fg(3),'facecolor',[0.5 0.5 1])
axis([0 time-1 0 max(time_hist_score_mix)+2000])
legend('Green','Shot', 'Blue')
title(titleq{1});
...
title({'^B^u^b^b^l^e Basic Score' ;['KEEP TIME: ' num2str(Stay_Time)]})
