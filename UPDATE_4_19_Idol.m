...
data2 = readtable(addr, 'Format', '%s%f%f%s%s', 'Delimiter', '\t', 'ReadVariableNames', true);
...
Total_Score_Mix(:, 1) = Total_Score_Long(:, 1) + Total_Score_Slip(:, 1) + Total_Score_Shot(:, 1);	%各类分数相加得出总分   
...
time_hist_score_dist = [time_hist_score_long';time_hist_score_shot';time_hist_score_slip']; %转换为图表数据格式
...
title({'^I^d^o^l Basic Score' ;['KEEP TIME: ' num2str(Stay_Time)]})
